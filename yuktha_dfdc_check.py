# -*- coding: utf-8 -*-
"""Yuktha_DFDC_check.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Q_J3xqpgLUTD7W1loaiWCCaJPniCtosI
"""
...
# ==========================================================
# DFDC FULL INFERENCE PIPELINE (Upload -> Predict)
# Compatible with your training + preprocessing structure
# ==========================================================

# -------------------- INSTALLS ----------------------------
# (Skip if already installed)
!pip install -q insightface timm librosa transformers onnxruntime
...
class Encoder(nn.Module):
    def __init__(self, in_dim, embed_dim):
        super().__init__()
        self.enc = make_mlp(in_dim, embed_dim=embed_dim)
...
# Feature dims from your training logs:
SPATIAL_DIM = 1792
MOTION_DIM  = 512
AUDIO_DIM   = 1536
PHYS_DIM    = 8
...
class PhysEncoder(nn.Module):
    def __init__(self, in_dim, embed_dim=64):
        super().__init__()
        self.enc = make_mlp(in_dim, hidden=256, embed_dim=embed_dim)  # hidden=256 !!!
        self.classifier = nn.Linear(embed_dim, 1)
    def forward(self, x):
        emb = self.enc(x)
        logit = self.classifier(emb).squeeze(1)
...
face_enc.load_state_dict(torch.load(f"{CKPT_DIR}/face_best.pth", map_location=DEVICE))
motion_enc.load_state_dict(torch.load(f"{CKPT_DIR}/motion_best.pth", map_location=DEVICE))
audio_enc.load_state_dict(torch.load(f"{CKPT_DIR}/audio_best.pth", map_location=DEVICE))
phys_enc.load_state_dict(torch.load(f"{CKPT_DIR}/phys_best.pth", map_location=DEVICE))
fusion.load_state_dict(torch.load(f"{CKPT_DIR}/fusion_best.pth", map_location=DEVICE))
...
# ==========================================================
# -------- FEATURE EXTRACTION (same logic as preprocessing)
# ==========================================================
...
def extract_features(video_path):
    ...
    try:
        ...
        if result.returncode != 0 or not os.path.exists(tmp_wav):
            print("⚠️ ffmpeg audio extraction failed. Using zero audio features.")
